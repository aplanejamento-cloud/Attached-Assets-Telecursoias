...
import { Search, Compass, BookOpen, Flame, User } from "lucide-react";
    ...
    <div className="min-h-[100dvh] flex flex-col bg-background text-foreground">
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-card/80 backdrop-blur-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <Link href="/feed" className="flex items-center gap-2 font-bold text-xl text-primary tracking-tight">
      ...
      <nav className="md:hidden fixed bottom-0 w-full border-t bg-card flex justify-around p-3 pb-safe z-50">
        <Link href="/feed" className={`flex flex-col items-center gap-1 ${location === '/feed' ? 'text-primary' : 'text-muted-foreground'}`}>
          <Compass className="w-5 h-5" />
          <span className="text-[10px] font-medium">Descobrir</span>
        </Link>
        <Link href="/feed" className="flex flex-col items-center gap-1 text-muted-foreground">

...
router.post("/api/ia-videos", auth, (req, res) => {
  const { titulo, descricao, url, iaId, thumbnail = "" } = req.body;

  if (!titulo || !descricao || !url || !iaId) {
    return res
      .status(400)
      .json({ error: "Título, descrição, URL e iaId são obrigatórios." });
  }
...
router.put("/api/ia-videos/:id", auth, (req, res) => {
  const id = Number(req.params.id);
...
router.delete("/api/ia-videos/:id", auth, (req, res) => {
  const id = Number(req.params.id);
  const videos = readVideos();
  const index = videos.findIndex((v) => v.id === id);
